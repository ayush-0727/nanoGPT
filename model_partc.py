...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, kvcache=None):
        ...
        k = k.view(B, curr_T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs) where nh = num heads, hs = head size
...
class Block(nn.Module):
    ...
    def forward(self, x, kvcache=None):
        attn_out, new_kv = self.attn(self.ln_1(x), kvcache)
        x = x + attn_out
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, past_kv=None, pos_offset=0):   # idx: (B,T) tensor of indices in the vocab where B is batch size, T is sequence length
        device = idx.device
        b, t = idx.size()
